...
//					showOnLoad: 'albums', 
				...
				*/
				...
				var $filters = $('#Filters').find('li'),
					dimensions = {
						records: 'all', // Create string for first dimension
						decade: 'all' // Create string for second dimension
					};
				...
				$filters.on('click',function(){
					...
					if(filter == 'all'){
						...
						if(!$t.hasClass('active')){
							// make the new one active AND remove active from former actives
							$t.addClass('active').siblings().removeClass('active');
							// CHANGE FILTER TO CURRENT SELECTION
							filterString = filter;
						} else {
							// Uncheck
							$t.removeClass('active');
							// THEN MAKE "ALL" ACTIVE
							filterString = 'all';
					...
					console.info('dimension 1: '+dimensions.records);
					console.info('dimension 2: '+dimensions.decade);
					...
					*/
					...
					$('#Grid').mixitup('filter',[dimensions.records, dimensions.decade])			
				});
